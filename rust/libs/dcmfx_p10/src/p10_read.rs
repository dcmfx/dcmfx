...
impl P10ReadContext {
  ...
  ///
  /// The default is 'Implicit VR Little Endian', which is what's specified by
  /// the DICOM Part 10 specification. However, when to DICOM P10 data that uses
  /// a known transfer syntax and has no File Meta Information, explicitly
  /// setting the fallback transfer syntax is necessary.
  ///
  /// This function should be called prior to reading any DICOM P10 parts from
  /// the read context.
  ///
  ...
  /// enough data to return the next part, i.e. further calls to
  ...
  fn read_data_element_value_bytes_part(
    ...
    // be read as a whole so use its full length as the number of bytes to read,
  ...
  fn is_materialized_value_required(
    ...
    // If the value is a string and it isn't UTF-8 data for an encoded string
    // that can be passed straight through then materialize it so that it can be
    ...
    // seen that incorrectly puts out of range characters into these fields, so
    // they are sanitized by replacing invalid characters with a question mark.
    vr.is_string()
      && !{
