...
5. Extract pixel data from a DICOM P10 file to an MP4 video:
   ...
   Additional options are available to control the CRF (i.e. video quality), the
   encoding preset, the FFmpeg log level, override the frame rate, and so on:

   ```sh
   dcmfx get-pixel-data input.dcm --format mp4 --mp4-crf 6 --mp4-preset slow
     --mp4-log-level info --mp4-fps 10
   ```
