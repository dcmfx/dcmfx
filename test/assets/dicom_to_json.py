...
# Writes the pixel data values to its own JSON file if the DICOM contains native
# pixel data.
def dicom_pixel_data_to_json(file, data_set):
    ...
    if (
        ...
        or data_set.file_meta.TransferSyntaxUID.is_encapsulated
    ):
        return
    ...
    if data_set.SamplesPerPixel == 1:
        if (
            ...
            frame_data = pydicom.pixels.apply_color_lut(data_set.pixel_array, data_set)
    ...
    elif data_set.SamplesPerPixel == 3:
        frame_data = (data_set.pixel_array / 255).tolist()
        if not isinstance(frame_data[0][0][0], list):
            frame_data = [frame_data]
