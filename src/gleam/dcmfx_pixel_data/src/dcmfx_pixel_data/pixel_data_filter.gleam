...
import dcmfx_pixel_data/raw_frame.{type PixelDataRawFrame}
...
pub fn add_part(
  ...
  part: P10Part,
) -> Result(#(List(PixelDataRawFrame), PixelDataFilter), DataError) {
      ...
      let frames = case filter.pixel_data |> deque.is_empty() {
        ...
        False -> {
          let frame =
            filter.pixel_data
            |> deque.to_list
            |> list.fold(raw_frame.new(), raw_frame.push_fragment)
...
fn get_pending_native_frames(
    ...
    < filter.native_pixel_data_frame_size
  {
    ...
    False -> {
      let #(frame, filter) = get_pending_native_frame(filter, raw_frame.new())
...
) -> #(PixelDataRawFrame, PixelDataFilter) {
  let frame_size = filter.native_pixel_data_frame_size
  let frame_length = raw_frame.length(frame)

  case raw_frame.length(frame) < frame_size {
    True -> {
      ...
      case chunk_length <= frame_size - frame_length {
        ...
        // to the frame
        True -> {
          let frame = raw_frame.push_fragment(frame, chunk)
        ...
        False -> {
          ...
          let frame = frame |> raw_frame.push_fragment(fragment)
          ...
          case number_of_frames > 1 {
            True -> {
              let frame =
                filter.pixel_data
                |> deque.to_list
                |> list.fold(raw_frame.new(), fn(frame, chunk) {
                  raw_frame.push_fragment(frame, chunk)
                })
...
fn get_pending_encapsulated_frames_using_offset_table(
  ...
  case offset_table {
    [#(_, frame_length), #(offset, _), ..] -> {
      ...
      let #(frame, filter) =
        get_pending_encapsulated_frame(filter, raw_frame.new(), offset)
  ...
  case filter.pixel_data_read_offset < next_offset {
    True ->
      case deque.pop_front(filter.pixel_data) {
        Ok(#(chunk, pixel_data)) -> {
          let frame = frame |> raw_frame.push_fragment(chunk)
...
fn apply_length_to_frame(
  frame: PixelDataRawFrame,
  frame_length: Int,
) -> Result(PixelDataRawFrame, DataError) {
  case raw_frame.length(frame) {
    len if len == frame_length -> Ok(frame)

    len if len > frame_length ->
      Ok(raw_frame.drop_end_bytes(frame, len - frame_length))
    ...
    _ ->
      Error(data_error.new_value_invalid(
        ...
        <> int.to_string(raw_frame.length(frame))
        <> "'",
      ))
  }
}
