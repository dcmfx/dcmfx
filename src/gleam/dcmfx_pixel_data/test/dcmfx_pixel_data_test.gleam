...
pub fn read_native_single_frame_test() {
  ...
  assert data_set.new()
    ...
    |> data_set.insert(dictionary.pixel_data.tag, pixel_data)
    |> dcmfx_pixel_data.get_pixel_data_frames
    == Ok([frame_with_fragments(0, [<<1, 2, 3, 4>>])])
}
...
pub fn read_native_multi_frame_with_one_bit_allocated_test() {
  ...
  assert pixel_data_frame.to_bytes(frame_0) == <<0b00110001, 0b00011011>>

  assert pixel_data_frame.to_bytes(frame_1) == <<0b01000110, 0b11001011, 0>>

  assert pixel_data_frame.to_bytes(frame_2) == <<0b01010101, 0b00011000, 0>>

  assert list.map(frames, pixel_data_frame.bit_offset) == [0, 7, 6]
}
...
pub fn read_native_multi_frame_with_one_bit_allocated_and_multiple_frames_in_one_byte_test() {
  ...
  assert pixel_data_frame.to_bytes(frame_0) == <<0b01010001>>

  assert pixel_data_frame.to_bytes(frame_1) == <<0b00001010>>

  assert pixel_data_frame.to_bytes(frame_2) == <<0b00110101, 0>>

  assert pixel_data_frame.to_bytes(frame_3) == <<0b00000110>>

  assert list.map(frames, pixel_data_frame.bit_offset) == [0, 3, 6, 1]
}
...
pub fn read_encapsulated_multiple_fragments_into_single_frame_test() {
  assert dcmfx_pixel_data.get_pixel_data_frames(data_set_with_three_fragments())
    == Ok([
...
pub fn read_encapsulated_multiple_fragments_into_multiple_frames_test() {
  ...
  assert dcmfx_pixel_data.get_pixel_data_frames(data_set)
    == Ok([
      frame_with_fragments(0, [
        string.repeat("1", 0x4C6) |> bit_array.from_string,
      ]),
...
pub fn read_encapsulated_using_extended_offset_table_test() {
  ...
  assert data_set_with_three_fragments()
    |> data_set.insert(
