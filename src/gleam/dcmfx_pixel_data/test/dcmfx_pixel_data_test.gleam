...
pub fn read_native_empty_frame_test() {
  ...
  |> dcmfx_pixel_data.get_pixel_data_raw_frames
  |> should.equal(Ok([]))
}
...
pub fn read_native_single_frame_test() {
  ...
  |> data_set.insert(dictionary.pixel_data.tag, pixel_data)
  |> dcmfx_pixel_data.get_pixel_data_raw_frames
  |> should.equal(Ok([raw_frame_with_fragments([<<1, 2, 3, 4>>])]))
}
...
pub fn read_encapsulated_multiple_fragments_into_multiple_frames_test() {
  ...
  |> result.then(dcmfx_pixel_data.get_pixel_data_raw_frames)
  |> should.equal(
    Ok([
      raw_frame_with_fragments([
        string.repeat("1", 0x4C6) |> bit_array.from_string,
...
fn raw_frame_with_fragments(fragments: List(BitArray)) -> PixelDataRawFrame {
  list.fold(fragments, raw_frame.new(), raw_frame.push_fragment)
}
