...
pub fn specific_character_set_test() {
  let assert Ok(_) = dcmfx_character_set.from_string("")

  let assert Ok(_) = dcmfx_character_set.from_string("ISO_IR 144")

  let assert Ok(_) = dcmfx_character_set.from_string("ISO IR 144")

  let assert Ok(_) = dcmfx_character_set.from_string("iso-ir 144")

  let assert Error(_) = dcmfx_character_set.from_string("\\ISO_IR 144")

  let assert Ok(_) = dcmfx_character_set.from_string("\\ISO 2022 IR 144")
  ...
  let assert Error(_) = dcmfx_character_set.from_string("GB18030\\ISO_IR 192")

  let assert Error(_) = dcmfx_character_set.from_string("ISO_IR 90210")
}
  ...
  |> list.each(fn(x) {
    ...
    |> list.each(fn(delimiter) {
      ...
      assert decode_bytes(
        ...
        == "ν" <> string.from_utf_codepoints([cp]) <> "í"
    })
    ...
    |> list.each(fn(non_delimiter) {
      ...
      assert decode_bytes(
        ...
        == "ν" <> string.from_utf_codepoints([cp]) <> "ν"
    })
  })
}
...
pub fn decode_bytes_multi_byte_with_extensions_test() {
  ...
  // X 0208 bytes
  assert decode_bytes("ISO 2022 IR 87", <<0x5C, 0x41>>, string_type.MultiValue)
    == "楞"
...
pub fn sanitize_default_charset_bytes_test() {
  assert dcmfx_character_set.sanitize_default_charset_bytes(<<>>) == <<>>

  assert dcmfx_character_set.sanitize_default_charset_bytes(<<0x40, 0x50, 0x60>>)
    == <<0x40, 0x50, 0x60>>

  assert dcmfx_character_set.sanitize_default_charset_bytes(<<0xDD, 0x50, 0x60>>)
    == <<0x3F, 0x50, 0x60>>

  assert dcmfx_character_set.sanitize_default_charset_bytes(<<0x40, 0xDD, 0x60>>)
    == <<0x40, 0x3F, 0x60>>

  assert dcmfx_character_set.sanitize_default_charset_bytes(<<0x40, 0x50, 0xDD>>)
    == <<0x40, 0x50, 0x3F>>

  assert dcmfx_character_set.sanitize_default_charset_bytes(<<0xDD, 0xDD, 0xDD>>)
    == <<0x3F, 0x3F, 0x3F>>
}
