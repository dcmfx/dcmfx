...
use dcmfx_core::{
  DataElementValue, DataError, DataSet, TransferSyntax, dictionary,
...
impl P10WriteContext {
  ...
  ///
  pub fn read_bytes(&mut self) -> Vec<Bytes> {
  ...
  ///
  fn token_to_bytes(&self, token: &P10Token) -> Result<Bytes, P10Error> {
    match token {
      ...
      P10Token::End => Ok(Bytes::default()),
    }
  }

  /// Serializes a data element header to [`Bytes`]. If a VR is not supplied
