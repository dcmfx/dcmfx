use crate::internal::lookup_table_16bit;
...
static LOOKUP_TABLE: [u16; 8836] = [
  ...
  0x30FC, 0x2014, 0x2010, 0xFF0F, 0xFF3C, 0x301C, 0x2016, 0xFF5C, 0x2026,
...
mod tests {
  ...
  fn decode_next_codepoint_test() {
    for (bytes, expected_codepoint) in [
      (vec![0x20], '\u{0020}'),
      (vec![0x21], '\u{FFFD}'),
      (vec![0x21, 0x21], '\u{3000}'),
      (vec![0x21, 0x6F], '\u{FFE5}'),
      (vec![0x24, 0x5D], '\u{307D}'),
      (vec![0x3B, 0x33], '\u{5C71}'),
      (vec![0x45, 0x44], '\u{7530}'),
      (vec![0x74, 0x26], '\u{7199}'),
    ] {
      assert_eq!(
