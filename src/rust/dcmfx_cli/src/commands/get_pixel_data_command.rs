...
use crate::mp4_encoder::{
  H264Profile, LogLevel, Mp4Encoder, VideoCodec, VideoCompressionPreset,
  VideoEncoderConfig,
};
...
pub struct GetPixelDataArgs {
  ...
  #[arg(
    ...
    help = "When the output format is 'mp4', specifies the video codec to use.",
    default_value_t = VideoCodec::H264
  )]
  mp4_codec: VideoCodec,

  #[arg(
    long,
    help_heading = "MP4 Encoding",
    help = "When the output format is 'mp4' and the codec is 'h264', specifies \
      the profile to use. The profile controls the features and capability of \
      the video stream.",
    default_value_t = H264Profile::Main
  )]
  mp4_h264_profile: H264Profile,
  ...
  #[arg(
    ...
    default_value_t = VideoCompressionPreset::Medium
  )]
  mp4_preset: VideoCompressionPreset,
  ...
  #[arg(
    ...
    help = "When the output format is 'mp4', specifies the constant rate \
      ...
      \n\
      The default CRF for H.264 is 18.",
...
fn create_mp4_encoder(
  ...
  let encoder_config = VideoEncoderConfig {
    ...
    h264_profile: args.mp4_h264_profile,
...
fn write_frame_to_mp4_file(
  ...
  // per frame.
  let frame_duration = if let Some(fps) = args.mp4_fps {
    Some(Duration::from_secs_f64(1.0 / fps))
  } else {
    cine_module.frame_duration(frame.index(), multiframe_module)
  };

  let frame_duration = frame_duration.unwrap_or(Duration::from_secs(1));
