...
compress_data (j_compress_ptr cinfo, JSAMPIMAGE input_buf)
{
  ...
  for (yoffset = diff->MCU_vert_offset; yoffset < diff->MCU_rows_per_iMCU_row;
    ...
    jdimension_result_t entropy_encode_mcus_result =
      (*losslsc->entropy_encode_mcus) (cinfo,
                       diff->diff_buf, (JDIMENSION)yoffset, MCU_col_num,
                       (JDIMENSION)cinfo->MCUs_per_row - MCU_col_num);
    if (entropy_encode_mcus_result.is_err) {
        return RESULT_ERR(boolean, entropy_encode_mcus_result.err_code);
    }
    MCU_count = entropy_encode_mcus_result.value;
...
compress_first_pass (j_compress_ptr cinfo, JSAMPIMAGE input_buf)
{
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
    ...
    if (access_virt_sarray_result.is_err) {
      return RESULT_ERR(boolean, access_virt_sarray_result.err_code);
    }
...
jinit_c_diff_controller (j_compress_ptr cinfo, boolean need_full_buffer)
{
  ...
  diff = (c_diff_ptr) alloc_small_result.value;
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
    ...
    diff->cur_row[ci] = *alloc_sarray_result.value;

    alloc_sarray_result = (*cinfo->mem->alloc_sarray)
      ((j_common_ptr) cinfo, JPOOL_IMAGE,
       (JDIMENSION) jround_up((long) compptr->width_in_data_units,
                  (long) compptr->h_samp_factor),
       (JDIMENSION) 1);
    diff->prev_row[ci] = *alloc_sarray_result.value;
  }

  /* Create the difference buffer. */
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
       ci++, compptr++) {
    jdiffarray_result_t alloc_darray_result = (*cinfo->mem->alloc_darray)
      ((j_common_ptr) cinfo, JPOOL_IMAGE,
       (JDIMENSION) jround_up((long) compptr->width_in_data_units,
                  (long) compptr->h_samp_factor),
  ...
  if (need_full_buffer) {
    ...
    for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
     ci++, compptr++) {
      jvirt_sarray_result_t request_virt_sarray_result = (*cinfo->mem->request_virt_sarray)
