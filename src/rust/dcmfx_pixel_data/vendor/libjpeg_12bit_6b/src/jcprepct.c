...
create_context_buffer (j_compress_ptr cinfo)
{
  ...
  fake_buffer = (JSAMPARRAY) alloc_small_result.value;
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
    ...
    true_buffer = alloc_sarray_result.value;
...
jinit_c_prep_controller (j_compress_ptr cinfo, boolean need_full_buffer)
{
  ...
  prep = (my_prep_ptr) alloc_small_result.value;
  ...
  if (cinfo->downsample->need_context_rows) {
    ...
    void_result_t create_context_buffer_result = create_context_buffer(cinfo);
    if (create_context_buffer_result.is_err) {
      return create_context_buffer_result;
    }
#else
    ...
    ((j_common_ptr) cinfo, JPOOL_IMAGE,
     ...
     prep->color_buf[ci] = alloc_sarray_result.value;
    }
  }
