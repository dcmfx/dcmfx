...
pub use raw_frame::PixelDataRawFrame;
...
where
  Self: Sized,
{
  /// Returns the frames of raw pixel data present in a data set.
  ///
...
impl DataSetPixelDataExtensions for DataSet {
  fn get_pixel_data_raw_frames(
    ...
    let mut filter = PixelDataFilter::new();

    // Create a new data set containing only the data elements needed by
    // PixelDataFilter. This avoids calling `DataSet::to_p10_parts()` on this
    // whole data set, which could be quite a bit slower if it has many data
    // elements.
    ...
    ds.to_p10_parts(&mut |part| {
      frames.extend_from_slice(&filter.add_part(part)?);
      Ok(())
    })?;
...
mod tests {
  ...
  fn read_native_empty_frame() {
    ...
    assert_eq!(ds.get_pixel_data_raw_frames().unwrap(), vec![],);
  }
  ...
  fn read_native_single_frame() {
    ...
    assert_eq!(
      ds.get_pixel_data_raw_frames().unwrap(),
      vec![raw_frame_with_fragments(&[&[1, 2, 3, 4]])],
    );
  }
  ...
  fn read_native_multi_frame() {
    ...
    assert_eq!(
      ds.get_pixel_data_raw_frames().unwrap(),
      vec![
        raw_frame_with_fragments(&[&[1, 2]]),
        raw_frame_with_fragments(&[&[3, 4]])
      ],
    );
  }
  ...
  fn read_native_multi_frame_malformed() {
    ...
    assert_eq!(
      ds.get_pixel_data_raw_frames().unwrap_err().to_string(),
  ...
  fn read_encapsulated_multiple_fragments_into_single_frame() {
    assert_eq!(
      data_set_with_three_fragments()
        .get_pixel_data_raw_frames()
        .unwrap(),
      vec![raw_frame_with_fragments(&[
  ...
  fn read_encapsulated_multiple_fragments_into_multiple_frames() {
    ...
    assert_eq!(
      ds.get_pixel_data_raw_frames().unwrap(),
      vec![
        raw_frame_with_fragments(&["1"
          .repeat(0x4C6)
          .as_bytes()
          .to_vec()
          .as_slice()]),
        raw_frame_with_fragments(&["2"
          .repeat(0x24A)
          .as_bytes()
          .to_vec()
          .as_slice()]),
        raw_frame_with_fragments(&["3"
          .repeat(0x628)
          .as_bytes()
          .to_vec()
          .as_slice()]),
  ...
  fn raw_frame_with_fragments(fragments: &[&[u8]]) -> PixelDataRawFrame {
